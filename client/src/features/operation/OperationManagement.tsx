...
import ModuleComp from '../module/components/ModuleComp';
...
export default function OperationManagement() {
  const [operations, setOperations] = useState<ModuleType[] | null>(null);
  ...
  const canCreate = can("rbac.operation.create");
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 flex flex-col gap-4 rounded-2xl border border-slate-200/60 bg-white/50 backdrop-blur-xl p-6 shadow-xl shadow-slate-200/20 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-900">Operations</h1>
            <p className="mt-2 text-sm text-slate-500">View and manage system operations available for RBAC.</p>
          </div>
        ...
        <div className="mb-4 flex items-center justify-between px-2">
          <h2 className="text-lg font-semibold text-slate-900">Available Operations</h2>
        ...
        {
          (!operations) ? (
            <FullPageLoader isLoading={!operations} z='z-0' text='Operations Loading...' />
          ) : operations.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-12 text-center">
              ...
              <p className="text-lg font-medium text-slate-900">No operations found</p>
              <p className="mt-1 text-sm text-slate-500">Get started by creating a new operation.</p>
            </div>
          ) : (
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {operations.map((operation) => (
                <ModuleComp key={operation._id} module={operation} />
              ))}
            </div>
          )
        }
